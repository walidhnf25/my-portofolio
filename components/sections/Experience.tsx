...
export default function Experience() {
  return (
    <section id="experience" className="relative py-10 sm:py-16 overflow-hidden bg-[#FAF8F5]">
              ...
              {experience.map((exp, index) => (
                ...
                >
                  ...
                  <div className="border-2 border-black p-4 sm:p-6 bg-white relative">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                          <p className="text-[9px] xs:text-[10px] font-bold tracking-[0.15em] sm:tracking-[0.2em] uppercase text-[#DA291C]">
                            {exp.period}
                          </p>
                          <span className="w-1 h-1 rounded-full bg-[#999]" />
                          <span className="text-[9px] xs:text-[10px] tracking-widest uppercase text-[#666]">
                            {exp.type}
                          </span>
                        </div>
                    ...
                    {/* Description */}
                    <p className="font-serif text-[13px] sm:text-sm text-[#444] leading-relaxed border-l-2 border-[#DA291C] pl-3 sm:pl-4 mb-3 sm:mb-4">
                      {exp.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1 sm:gap-2">
                      {exp.highlights.map((highlight) => (
                        <span
                          key={highlight}
                          className="text-[8px] xs:text-[9px] sm:text-[10px] px-2 sm:px-3 py-1 border border-[#ddd] text-[#333] hover:border-[#DA291C] hover:text-[#DA291C] transition-colors"
                        >
                          {highlight}
                        </span>
                      ))}
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="border-t border-[#333] pt-3 sm:pt-4 grid grid-cols-2 gap-3">
                  <div className="text-center">
                    ...
                    <p className="text-[8px] xs:text-[9px] sm:text-[10px] uppercase text-[#666]">Internships</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg sm:text-xl font-bold text-white">2</p>
                    <p className="text-[8px] xs:text-[9px] sm:text-[10px] uppercase text-[#666]">Part-Time</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between">
                    <span className="font-serif text-[13px] sm:text-sm text-black">{project.name}</span>
                    <span className="text-[8px] xs:text-[9px] sm:text-[10px] text-[#DA291C]">{project.type}</span>
                  </div>
                ))}
              </div>
            </div>
